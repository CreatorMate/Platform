...
<template>
    <div class="w-full py-8 border-b border-gray-500 border-opacity-10 flex items-center">
        ...
        <div class="flex items-center gap-3 font-semibold">
            <nuxt-img class="rounded-xl w-16 h-16" :src="accountState.user?.picture ? 'https://mfouoyeneddsfujxfjci.supabase.co/storage/v1/object/public/user_pictures/' + accountState.user.picture : user.userInfo.picture"></nuxt-img>
            <input ref="fileInput" type="file" accept="image/*" @change="selectImage" style="display: none"/>
            <button :disabled="!accountState.user?.picture" class="hover:underline transition duration-200" @click="deleteFoto">Delete</button>
            <button @click="fileInput.click()" class="text-blue-700 hover:text-blue-900 transition duration-200">Update</button>
        </div>
    </div>
    <div class="w-full py-3 border-b border-gray-500 border-opacity-10 flex flex-col justify-center gap-3">
        <div class="flex items-center w-full">
            ...
            <input @focusout="saveInput" v-model="fullName" class="flex flex-grow shadow px-6 py-2 rounded-3xl border border-black border-opacity-20" type="text"/>
        </div>
        <div class="flex items-center w-full">
            <p class="text-text-normal font-semibold w-[350px]">Email</p>
            <input @focusout="saveInput" v-model="email" class="flex flex-grow shadow px-6 py-2 rounded-3xl border border-black border-opacity-20" type="text"/>
        </div>
    </div>
</template>